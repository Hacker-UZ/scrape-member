...
' ╚═╝░░░░░╚═╝░░╚═╝╚══════╝╚══════╝ ',
    ...
    print(f' Version: 1.0 | Author: @kamal939{rs}\n')
...
try:
    ...
    with open('status.dat', 'rb') as f:
        ...
        f.close()
        lol = input(f'{INPUT}{cy} Resume scraping members from {w}{status[0]}{lg}? [y/n]: {w}')
        if 'y' in lol:
            ...
            scraped_grp = input(f'{INPUT}{cy} Public/Private group link to scrape members: {w}')
            index = 0
except:
    scraped_grp = input(f'{INPUT}{cy} Public/Private group link to scrape members: {w}')
    index = 0
...
print(f'{info}{lg} Total accounts: {r}{len(accounts)}')
number_of_accs = int(input(f'{INPUT}{cy} Enter number of accounts to use: {w}'))
print(f'{info}{cy} Choose an option{lg}')
print(f'{cy}[0]{lg} Add to public group')
print(f'{cy}[1]{lg} Add to private group')
choice = int(input(f'{INPUT}{cy} Enter choice: {w}'))
if choice == 0:
    target = str(input(f'{INPUT}{cy} Enter public group link: {w}'))
else:
    target = str(input(f'{INPUT}{cy} Enter private group link: {w}'))
print(f'{grey}_'*50)
...
with open('vars.txt', 'wb') as f:
    ...
    for ab in to_use:
        pickle.dump(ab, f)
    f.close()
sleep_time = int(input(f'{INPUT}{cy} Enter delay time per request{w}[{lg}0 for None{w}]: {w}'))
